...
"""Delete command for the Label Manager - Label Values CLI."""
...
class Delete(base.Command):
  """Deletes the specified LabelValue resource.

    Deletes the LabelValue resource given the LabelValue's display name
  ...
  detailed_help = {
      'EXAMPLES':
          """
          To delete a LabelValue with id '123', run:

            $ {command} labelValues/123

          To delete a LabelValue with the name 'prod' that lives under
          'labelKeys/456', run:

            $ {command} prod --label-key='labelKeys/456'

          To delete a LabelValue with the name 'prod' under LabelKey 'env' that
  ...
  def Run(self, args):
    ...
    if args.async_:
      ...
      return operations.WaitForOperation(
          op, 'Waiting for LabelValue [{}] to be deleted with [{}]'.format(
