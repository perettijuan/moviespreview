 ...
 *
 * [VM] represents the main [MPViewModel] that the Fragment uses to provide the functionallity
 * requested to the user.
 *
 * Navigation support:
 * [MPFragment] works in conjunction with [MPViewModel] to update the AppBar title each time
 * a new screen is shown. Basically, the [MPViewModel] that is taking control of the application
 * posts a new event to [MPViewModel.destinationEvents] in order to indicate that a new destination
 ...
 * works with [MPViewModel] reacting to navigation updates posted in [MPViewModel.navigationEvents].
 * Every time [MPViewModel] posts a new destination to navigate to, [MPFragment] posts a new
 ...
 */
abstract class MPFragment<VM : MPViewModel> : Fragment() {
